        ...
        while True:
            ...
            if req_id not in self._reqs_to_send and req_id not in self._reqs_to_process:
                if (meta := self._recving_metadata.get(req_id)) is not None:
                    # We are acting as the consumer. One notif per producer
                    # rank that writes here: the producer stages holding layers
                    # we also hold, times the producer TP ranks mapped to us
                    # (>1 when producer TP > consumer TP; tp_size is producer
                    # TP, world_size is ours and is TP-only).
                    #
                    # The stage term is not simply meta.pp_size. A PP-sharded
                    # consumer is written to only by the producer stages whose
                    # layer window overlaps its own -- with equal splits on both
                    # sides that is exactly one, and expecting meta.pp_size
                    # would wait forever for notifs no one sends.
                    producers_per_consumer = max(1, int(tp_size) // self.world_size)
                    expected_notifs = (
                        self._overlapping_remote_pp_stages(meta.pp_size)
